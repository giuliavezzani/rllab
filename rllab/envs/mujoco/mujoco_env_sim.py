...
from mujoco_py import load_model_from_path, MjSim, MjModel, MjViewer
...
class MujocoEnv(Env):
    ...
    def __init__(self, action_noise=0.0, file_path=None, template_args=None):
        ...
        self.model_for_rendering = load_model_from_path(file_path)
        import IPython
        IPython.embed()
        self.sim = MjSim(self.model_for_rendering)
    ...
    def render(self, close=False, mode='human'):
        if mode == 'human':
            ...
            #import IPython
            #IPython.embed()
            img = self.sim.render(500, 500)
            print(img)
            return np.fromstring(img, dtype='uint8').reshape(height, width, 3)[::-1,:,:]
        if close:
            self.stop_viewer()
